...
function ponychanx() {
	...
	var QR = {
		...
		show: function() {
			...
			$jq("body").append(qr);
			$jq("#qr .close a").on("click", QR.hide);
			...
			if (Settings.gets("Hide quick reply when top button clicked")) $jq("#qr .top a:first").on("click", QR.hide);
			var btn = $jq("#qr > input[type='button']");
			if (Main.tid == "0") btn.val("Thread"); 
			btn.on("click", QR.send);
		...
		send: function() {
			...
			if (!$jq("input[name='nofile']", "#qr").is(":checked") && si != null) d.append("imagefile", si);
		...
		add: function() {
			QR.settitle("");
			var fl = document.getElementById("imgfile").files;
			if (fl.length == 0) return;
			var url = window.URL || window.webkitURL;
			var max = $jq("#postform input[name='MAX_FILE_SIZE']").val();
			for (var i = 0, len = fl.length; i < len; i++) {
				var f = fl[i];
				if (f.size > max) {
					QR.settitle(f.name + " is too large");
					continue;
				}
				if (!/^image/.test(f.type)) {
					QR.settitle(f.name + " is not an image");
					continue;
				}
				QR.images.push(f);
				var sf = QR.images[QR.images.length-1];
	...
	var Posts = {
		...
		newhandle: function(p) {
			...
			if (im != null) {
				...
				if (Settings.gets("Add save image shortcut to posts")) {
					...
					var fnm = fst.text().match(/.*.(gif|png|jpg|mp3|docx|txt)/g);
					if (fnm != null) {
		...
		newpostupdate: function(p) {
			try {
				...
				$jq(".posttime",p).html(Date.parse($jq(".posttime",p).text()).addHours(8 + timezone).toString(timeFormat).replace(/([AP]M)$/, '<span style="font-size:0.75em">$1</span>'));
			} catch (e) { }
	...
	var Html = {
		...
		seticon: function(i) {
			var si = $jq("link[rel='shortcut icon']", "head");
			if (si.attr("href") != i) {
				si.remove();
				$jq("<link rel='shortcut icon' type='image/x-icon' />").attr("href", i).appendTo("head");
			}
		},
		settitle: function(t) {
			document.title = t;
		...
		css: function() {
			...
			#qr textarea { min-width: 394px; padding: 2px 0 2px 4px; font-family: sans-serif; height: 98px; font-size: small; }\
	...
	var Notifier = {
		...
		scroll: function() {
			...
			Html.seticon(Notifier.unread.length > 0 ? Html.iconunread : Html.iconread);
	...
	var Settings = {
		...
		settings: {
			...
			"Enable hide post buttons": { def: "false", cat: "Posts" },
