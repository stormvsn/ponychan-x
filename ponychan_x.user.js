...
Css = {
	init: function() {
		...
		#qr input[type='checkbox'], #settingsWrapper input[type='checkbox'] { position: relative; top: 2px; }\
...
Favicon = {
	...
	dead: "data:image/png;base64,",
	init: function() {
		Favicon.set();
	},
	set: function(icon) {
...
QR = {
	...
	init: function() {
		...
		$j("<input />").attr("type", "text").attr("name", "name").attr("placeholder", "Name").val(Settings.get("qr.name") || "").appendTo(QR.el);
...
Title = {
	...
	update: function() {
		...
		Title.task = setTimeout(function() {
			document.title = "(" + (Main.status == 404 ? "404" : Title.unread.length) + ") " + Title.title;
			if (Set["Enable favicons"])
				Favicon.set(Main.status == 404 ? Favicon.dead : Title.unread.length ? Favicon.unread : Favicon.read);
		}, 50);
	},
	scroll: function() {
...
Time = {
	...
	node: function(post) {
		...
		if (minutes < 10)
			minutes = "0" + minutes;
		timeel.html(Time.days[d.getDay()] + ", " + Time.months[d.getMonth()] + " " + d.getDate() + ", " + d.getFullYear() + " " + hours + ":" + minutes + (twelvehour ? " <span style='font-size:0.75em'>" + (d.getHours() < 12 ? "AM" : "PM") + "</span>" : ""));
	}
};
